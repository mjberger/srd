...
In the original method, the second term on the right-hand side
of \eqref{eqn:taylor}  was approximated for example at edge
$j-1/2$ by solving a Riemann
problem between the centroid values of  $i,j$ and $i,j-1$,
evaluating the flux $G$ using the solution to produce $G_{i,j-1/2}$, 
and then differencing
$(\mathbf{G}_{i,j+1/2} - \mathbf{G}_{i,j-1/2})/\Delta y$ to
compute $\partial G_{i,j}/\partial y$. On a uniform mesh 
the first order accurate errors in computing $G$
in this procedure cancel, and the term itself is 
...
transverse derivative when computing the flux $F$ in the $i$ direction.
First,
...
transverese ($y$) direction. Instead, for all cut cells we
instead compute
...
point centered stencil to compute $u_x$ and $v_y$, and take the
max over a $3 \times  3$
...
L(\mathbf{U}^{n}))$, where $L$ represents the MUSCL operator.
...
& \quad \; \sigma^n_{xy,i,j}[(x_{r,s} - x_{i,j})(y_{r,s} - y_{i,j})-S_{i,x,y}] +\\
...
- fit a least squares polynomial in the $5 \times 5$ neighborhood centered on that cell.
