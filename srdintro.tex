...
although the small cells do not mimic the cut cells at the boundary in higher dimensions. 
...
Equation \eqref{eq:hpde} is discretized using the  first order upwind finite volume method.  
...
step $\Delta t$ is constant for all cells, and  $h_i = h$ when $i \neq -1,1$ and $h_i = \alpha h$ for $i = -1,1$.  
