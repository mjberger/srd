...
We illustrate state redistribution on a simple example in one dimension to gain some intuition.  
We solve the linear advection equation
\begin{equation}\label{eq:hpde}
u_t + au_x = 0, \quad a>0
\end{equation}
on the nonuniform grid, called the base grid, shown in Figure \ref{fig:ng1}.
This model problem illustrates the main ideas of SRD without too much notation, 
although the small cells do not represent the cut cells at the boundary in higher dimensions. 
...
\begin{center}
\vspace*{.2in}
\includegraphics[width=0.75\linewidth]{figs/overlapping.pdf} 
\caption{\sf Model problem in one space dimension on nonuniform base grid, with two small 
cells  at $i = -1$ and $1$.  The small and large cell 
sizes are $\alpha h$ ($0<\alpha \le 1$), and $h$, respectively. \label{fig:ng1}}
\end{center}
\end{figure}

Eq~\eqref{eq:hpde} is discretized using the  first order upwind finite volume method.  
After one forward Euler time step, the numerical solution is given by
...
We call the finite volume method \eqref{eq:unstable1d} the base scheme.  
The maximum stable time step of the base finite volume scheme on this grid would 
be $\Delta t \leq \alpha h/a$, which could be arbitrarily small.  

The update step \eqref{eq:unstable1d} is followed by the state redistribution post-processing
procedures that acts on the base scheme and stabilizes
$\widehat{U}_i$ in a conservative manner.  This is done by temporarily merging cells of the grid into 
larger, possibly overlapping, neighborhoods and recombining the solution averages on these 
neighborhoods in a particular fashion.
These merged cells are constructed once during a mesh preprocessing step before timestepping.

\begin{figure}[h]
\begin{center}
\includegraphics[width=0.75\linewidth]{figs/overlapping1.pdf} 
\caption{\sf Each blue arrow indicates the merging neighborhoods associated to the large cells $-3$, $-2$, 
$0$, $2$, and $3$.  The number above each blue arrow is the neighborhood index.
The red arrow corresponds to the neighborhood of small cells $-1$ and $1$, where the number below
each red arrow is the neighborhood index. \label{fig:mn1}}
\end{center}
\end{figure}
...
where eace cell finds a set of neighbors with which to temporarily merge.
...
composed of itself.  This is illustrated by the blue arrows in Figure \ref{fig:mn1} For example,  the 
...
merging neighborhood, weighted by number of neighborhoods. 
...
\end{equation}
In formula \eqref{eq:neigh2}  for the weighted solution average,
...
These multipliers are then divided by the weight volume,  $(h/2 + \alpha h + h/3)$.
...
with flux redistribution, so we are not concerned about the application of SRD in
higher dimensions and more complicated problems. The computational examples in section
\ref{sec:compResults} bear this out.  
...
We will demonstrate that the maximum stable time step is not restricted by the small cells,
and that the state redistribution method is conservative,
$$
 \sum_{i}h_i U^{n+1} = \sum_{i}h_i \widehat{U}_i =  \sum_{i}h_i U^{n} .
$$
