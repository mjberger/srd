...
volume-weighted merged cell $Q_0$:
\begin{equation}
Q_0 = \frac{\widehat{U}_{-1} + \alpha \,  \widehat{U_0} + 
\widehat{U_1}}{2+\alpha} .
\end{equation}
The cells comprising the merged cell
are then replaced by $Q_0$:
\begin{equation}
U_{-1}^{n+1} = U_0^{n+1} = U_1^{n+1} = Q_0
\end{equation}
This is easily seen to be conservative by checking that
\begin{equation}
U_{-1}^{n+1} + \alpha \, U_0^{n+1} + U_1^{n+1} = (2+\alpha) \, Q_0 =
\widehat{U}_{-1} + \alpha \, \widehat{U}_{0} +  \widehat{U}_{1}
\end{equation}
which equals the values at time $n$  except for the flux in and out of this
region.
...
might be to make two merging neighborhoods, $Q_{-1}$
comprising $u_{-2},u_{-1},u_0$, and $Q_1$ comprising $u_0, u_1$ and $u_2$.
...
to assign $u_0 = \frac{1}{2} (Q_{-1} + Q_1).$  It can quickly be  seen however
