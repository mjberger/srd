...
A least squares procedure  similar to the one used 
for the finite volume update of the cut cells is an 
obvious choice.  In eq. \eqref{eqn:lls} the $\sigma_x$
and $\sigma_y$ is replaced by $\widehat{\sigma_x}$ and
$\widehat{\sigma_y}$ indicating that the gradient is over the merging
tile now. We also use the merging tile's weighted centroid instead of 
the cell centroid. Otherwise the linear least squares procedure is identical.
The same limiting procedure (BJ or LP) is also applied to the
reconstruction solution on the merging neighborhood.
NEED TO TRY THIS
...
\end{equation}
where $\widehat{Q}_{i, j}$ is the solution average on the merging neighborhood in \eqref{tiledef} and $(\widehat{\sigma}_{x,i,j},\widehat{\sigma}_{y,i,j})$ is 
...
on the cut cells and adjacent full cells.} 
...
The final update formula \eqref{eq:final_update_linear} can easily be implemented with a 
