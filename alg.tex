...
For moving geometries they would need to be be done at each step.
...
In general, the weighted volume is not the physical volume of merging
neighborhood, nor is the weighted centroid and the physical centroid
the same, unless there is only one cell in the merging neighborhood
($\lvert M_{i,j} \rvert =1$)  and no other neighborhoods overlap that cell 
 ($N_{i,j}=1$).  
DID I GET THE INDICES RIGHT OR IS IT $N_{rs}$ PLEASE CHECK
IS CELL i,j INCLUDED in $M_i,j$?
...
If all cells had the same number of neighborhoods the $N_{ij}$ would all cancel. 
