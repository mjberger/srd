...
%\begin{equation}
%\left\lvert \lvert  \rho - \rho(x,y) \right\rvert \rvert_1 
%= \left \lvert \lvert e_{ij} \right \rvert \rvert_1  = \sum_{i,j} \,
%V_{ij} \lvert e_{ij } \rvert
%\label{eqn:L1vol}
%\end{equation}
%and the norm along the boundary cells only is
%\begin{equation}
%\left\lvert \rho - \rho(x,y) \right \rvert  = \left\lvert
%e_{ij} \right\rvert  = \sum_{{i,j} \in \partial} \, b_{ij} \lvert e_{ij
%} \rvert
%\label{eqn:L1bndry}
%\end{equation}
%where $b_{ij}$ is the length of the boundary segment in cut cell $(i,j)$.
...
Figure \ref{fig:cyl1} (left) shows the solution density and pressure  at time $t=0.25$,
